...
{
    public class CompressWriter : IWriter
    {
        ...
        public CompressWriter(SyncQueue writerQueue, Information information, string destinationPath)
        {
            ...
            using (var fileStream = new FileStream(_destinationPath, FileMode.Append))
            {
                try
                {
                    while (!_isFinished && _information.FinishFlag != 1)
                    {
                        ...
                        if (block != null)
                        {
                            var blockLength = BitConverter.GetBytes(block.Buffer.Length);
                            var position = BitConverter.GetBytes(block.Position);
                            fileStream.Write(blockLength, 0, 4);
                            fileStream.Write(position, 0, 8);
